...
const SidebarContent = (props) => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "https://ypsresttest.azurewebsites.net/Service1.svc/getQuerysUser/0fbd4533-b9f1-47df-a33c-c6c0a2bc366c/1679ce85-5abb-4d05-b795-1842901f850c"
        );
  ...
  const groupedMenuItems = menuItems.reduce((grouped, currentItem) => {
    const groupName = currentItem.GRUPONOMBRE;
    if (!grouped[groupName]) {
      grouped[groupName] = [];
    }
    grouped[groupName].push(currentItem);
    return grouped;
  }, {});

  // Mapeo de elementos de menú fuera del return
  const consultasMenu = Object.entries(groupedMenuItems).map(([groupName, groupItems]) => (
    <li key={groupName}>
      ...
      <ul className="sub-menu" aria-expanded="false">
        {groupItems.map((item) => (
          <li key={item.ID}>
            <Link to={`/queries/${item.ID}`}>{item.NOMBRE}</Link>
          </li>
        ))}
      </ul>
    </li>
          ...
          <ul className="metismenu list-unstyled" id="side-menu">
            ...
            <li>
              ...
              <ul className="sub-menu" aria-expanded="false">
                ...
                <li>
                  <Link to="/agcharts">{props.t("AgCharts")}</Link>
                </li>
                <li>
            ...
            {/* <li className="menu-title">{props.t("Consultas")} </li>
            {Object.entries(groupedMenuItems).map(([groupName, groupItems]) => (
              <li key={groupName}>
                <Link to="/#" className="has-arrow">
                  <i className="bx bx-home-circle"></i>
                  <span>{groupName}</span>
